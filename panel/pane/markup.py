...
from ..models.markup import HTML as _BkHTML
from ..models.markup import JSON as _BkJSON
from ..models.markup import HTMLStreamEvent
...
class HTMLBasePane(ModelPane):
    ...
    enable_streaming = param.Boolean(
        default=False,
        doc="""
...
class HTML(HTMLBasePane):
    ...
    sanitize_html = param.Boolean(
        default=False,
        doc="""
        Whether to sanitize HTML sent to the frontend.""",
    )

    sanitize_hook = param.Callable(
        default=HTML_SANITIZER.clean,
        doc="""
    ...
    _rename: ClassVar[Mapping[str, str | None]] = {"sanitize_html": None, "sanitize_hook": None, "stream": None}

    _rerender_params: ClassVar[list[str]] = ["object", "sanitize_html", "sanitize_hook"]

    @classmethod
    def applies(cls, obj: Any) -> float | bool | None:
        module, name = getattr(obj, "__module__", ""), type(obj).__name__
        if (any(m in module for m in ("pandas", "dask")) and name in ("DataFrame", "Series")) or hasattr(obj, "_repr_html_"):
...
class DataFrame(HTML):
    ...
    bold_rows = param.Boolean(
        default=True,
        doc="""
        Make the row labels bold in the output.""",
    )

    border = param.Integer(
        default=0,
        doc="""
    ...
    decimal = param.String(
        default=".",
        doc="""
        Character recognized as decimal separator, e.g. ',' in Europe.""",
    )

    escape = param.Boolean(
        default=True,
        doc="""
    ...
    formatters = param.ClassSelector(
        default=None,
        class_=(dict, list),
        doc="""
    ...
    index_names = param.Boolean(
        default=True,
        doc="""
        Prints the names of the indexes.""",
    )

    justify = param.Selector(
        default=None,
        allow_None=True,
        objects=["left", "right", "center", "justify", "justify-all", "start", "end", "inherit", "match-parent", "initial", "unset"],
        doc="""
    ...
    render_links = param.Boolean(
        default=False,
        doc="""
        Convert URLs to HTML links.""",
    )

    show_dimensions = param.Boolean(
        default=False,
        doc="""
    ...
    _rename: ClassVar[Mapping[str, str | None]] = {rp: None for rp in _rerender_params[1:-1]}

    _stylesheets: ClassVar[list[str]] = [f"{CDN_DIST}css/dataframe.css"]
    ...
    def applies(cls, obj: Any) -> float | bool | None:
        ...
        if any(m in module for m in ("pandas", "dask", "streamz", "geopandas", "spatialpandas")) and name in (
            "DataFrame",
            "Series",
            "Random",
            "DataFrames",
    ...
    def _transform_object(self, obj: Any) -> dict[str, Any]:
        ...
        if hasattr(obj, "to_html"):
            ...
            if "dask" in module:
                ...
                kwargs = {p: getattr(self, p) for p in self._rerender_params if p not in HTMLBasePane.param and p not in ("_object", "text_align")}
...
class Str(HTMLBasePane):
    ...
    _target_transforms: ClassVar[Mapping[str, str | None]] = {"object": """JSON.stringify(value).replace(/,/g, ", ").replace(/:/g, ": ")"""}
...
class Markdown(HTMLBasePane):
    ...
    extensions = param.List(
        default=["extra", "smarty", "codehilite"],
        nested_refs=True,
        doc="""
    ...
    renderer = param.Selector(
        default="markdown-it",
        objects=["markdown-it", "myst", "markdown"],
        doc="""
        Markdown renderer implementation.""",
    )

    renderer_options = param.Dict(
        default={},
        nested_refs=True,
        doc="""
    ...
    _rerender_params: ClassVar[list[str]] = ["object", "dedent", "extensions", "css_classes", "plugins", "disable_anchors"]

    _target_transforms: ClassVar[Mapping[str, str | None]] = {"object": None}

    _stylesheets: ClassVar[list[str]] = [f"{CDN_DIST}css/markdown.css"]
    ...
    def _get_parser(cls, renderer, plugins, hard_line_break=False, disable_anchors=True, **renderer_options):
        ...
        if renderer == "markdown-it":
            ...
            parser = MarkdownIt("gfm-like", renderer_cls=RendererHTML, options_update=renderer_options)
        elif renderer == "myst":
            from myst_parser.parsers.mdit import (MdParserConfig,
                                                  create_md_parser)

            config = MdParserConfig(
                heading_anchors=1, enable_extensions=["colon_fence", "linkify", "smartquotes", "tasklist", "attrs_block"], enable_checkboxes=True, **renderer_options
            )
            parser = create_md_parser(config, RendererHTML)
        parser = parser.enable("strikethrough").enable("table").use(deflist_plugin).use(footnote_plugin).use(tasklists_plugin)
    ...
    def _transform_object(self, obj: Any) -> dict[str, Any]:
        ...
        if self.renderer == "markdown":
            ...
            html = markdown.markdown(obj, extensions=extensions, output_format="xhtml", **self.renderer_options)
        else:
            parser = self._get_parser(self.renderer, tuple(self.plugins), self.hard_line_break, self.disable_anchors, **self.renderer_options)
            try:
...
class JSON(HTMLBasePane):
    ...
    _rerender_params: ClassVar[list[str]] = ["object", "depth", "encoder", "hover_preview", "theme"]

    _stylesheets: ClassVar[list[str]] = [f"{CDN_DIST}css/json.css"]
