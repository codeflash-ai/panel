...
def isurl(obj: Any, formats: Iterable[str] | None = None) -> bool:
    if not isinstance(obj, str):
        return False
    obj_lower = obj.lower()
    lower_string = obj_lower.partition("?")[0].partition("#")[0]
    if not lower_string.startswith(("http://", "https://")):
        return False
    if formats is None:
        return True
    fmt_tuple = tuple(f".{fmt}" for fmt in formats)
    return lower_string.endswith(fmt_tuple)
...
def isdatetime(value) -> bool:
    ...
    if is_series(value) and len(value):
        ...
        return value.dtype.kind == "M" or (value.dtype.kind == "O" and len(value) != 0 and isinstance(np.take(value, 0), datetime_types))
